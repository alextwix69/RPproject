...
- профиль игрока хранится в `users` и расширяется через `profile_data`;
- поиск друзей работает по RoleHub имени или Telegram username;
...
## Обязательная Стилистика Текста

Весь пользовательский текст RoleHub должен быть написан в едином **childish-majestic vibe**.
Это обязательное правило для новых и изменяемых экранов, кнопок, подсказок ввода, уведомлений, подтверждений и понятных пользователю ошибок.

Основные требования:

- обращаться к пользователю как к герою, исследователю миров или участнику приключения;
- использовать образы волшебства, королевств, порталов, вселенных, сокровищ, историй и титулов;
- добавлять уместные эмодзи в заголовки, кнопки и важные строки: `✨`, `👑`, `🌌`, `🌟`, `🏰`, `💎`, `🎭`, `🔮`;
- писать коротко, дружелюбно и понятно для аудитории 12-14 лет;
- сохранять точный смысл действия: по тексту кнопки должно быть ясно, что произойдёт после нажатия;
- не использовать сюсюканье, сложные метафоры или слишком много эмодзи, мешающих чтению;
- технические команды и админские данные оставлять точными, но их заголовки и навигацию также оформлять в общей стилистике.

Примеры:

```text
Плохо:  Создать лобби
Хорошо: ✨ Создать свой мир

Плохо:  Пользователь не найден
Хорошо: 🌙 Герой не найден. Проверь имя и попробуй ещё раз ✨
```

Перед добавлением нового пользовательского текста нужно проверить, что он одновременно понятный, короткий и соответствует этой стилистике.
...
- `avatar_file_id` хранит Telegram `file_id` фото профиля.
- `profile_data` оставлен как JSON-расширение профиля для будущих полей без немедленного добавления колонок.
...
### `src/models/friendship.py`

Модель `Friendship`.

Хранит связь между двумя пользователями для сценария друзей:
...
Пара `requester_id + addressee_id` уникальна. Проверка существующей связи делается в обе стороны через repository.
...
### `src/repositories/friendship_repo.py`

Операции со связями друзей.
...
Сервис профилей и действий друзей.

Ключевые функции:

- `set_avatar_file_id()` сохраняет Telegram `file_id` фото профиля;
- `find_user_for_friend_search()` ищет пользователя по RoleHub имени или Telegram username;
- `add_friend()` создаёт заявку в друзья и не даёт добавить себя или продублировать связь;
- `render_profile_text()` собирает текст карточки профиля.

Профиль сейчас показывает:

- имя RoleHub;
- Telegram username;
- состояние аватара;
- время последнего онлайн по `User.last_seen_at`.
...
Игровая кнопка `🌌 Войти в мир` делегируется в `src/handlers/play_lobby.py`.
Кнопка `🤝 Друзья и герои` открывает поиск пользователя по нику. После найденного пользователя доступны действия:

- `🤝 Добавить в друзья`;
- `👤 Открыть профиль`.

В разделе поддержки доступна одна рабочая кнопка `👑 Связаться с администратором`.
Она показывает inline-ссылку на Telegram-аккаунт `@sanyasigma2006`. FAQ, правила и баг-репорты из активного меню пока убраны.
Разделы shop/settings по-прежнему частично показывают заглушки.
...
```text
кнопка `🔎 Найти героя` при создании
...
```text
кнопка `🔎 Найти героя` при входе в лобби
...
```text
Пользователь нажимает `🌌 Войти в мир`
...
```text
Пользователь нажимает `🤝 Друзья и герои`
  -> `🔎 Найти героя`
  -> pending_action = find_friend
  -> следующее текстовое сообщение ищет пользователя по display_name или username
  -> экран найденного пользователя
  -> `👤 Открыть профиль` показывает карточку профиля
  -> `🤝 Добавить в друзья` создаёт pending-заявку Friendship
```
...
Настройки
  -> Профиль
  -> `✏️ Имя` меняет display_name
  -> `🖼 Аватар` ставит pending_action = set_profile_photo
  -> следующее фото сохраняется как users.avatar_file_id
```
...
- Профиль игрока сейчас состоит из RoleHub имени, аватара/фото и времени последнего онлайн.
- Дополнительные поля профиля можно добавлять через `users.profile_data` или отдельные таблицы, если появится сложная логика.
